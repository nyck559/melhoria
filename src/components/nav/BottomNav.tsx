...
export type ScreenKey = 'crescimento' | 'checklist' | 'recompensas' | 'calendario'
...
export default function BottomNav({ active, onChange }: { active: ScreenKey; onChange: (k: ScreenKey) => void }) {
  return (
    <nav className="absolute inset-x-3 bottom-3 z-30 grid grid-cols-4 gap-1 rounded-2xl border border-line bg-surface/95 p-1.5 backdrop-blur">
      {ITEMS.map((it) => {
        ...
        return (
          <button key={it.key} onClick={() => onChange(it.key)} className="relative flex flex-col items-center gap-1 rounded-xl py-2">
            ...
            <span className="relative text-[9.5px] font-semibold" style={{ color: on ? 'var(--blue)' : 'var(--muted)' }}>{it.label}</span>
          </button>
        )
      })}
    </nav>
